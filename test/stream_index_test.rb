    ...
    describe StreamIndex do
        ...
        describe "#base_time" do
            it "sets the base time to the given value" do
                stream_index.add_sample(0, base_time = 10)
                stream_index.base_time = base_time - 1
                assert_equal base_time - 1, stream_index.base_time
            end
            it "offsets the internal time representation if needed" do
                stream_index.add_sample(0, base_time = 10)
                stream_index.base_time = base_time - 1
                assert_equal 1, stream_index.internal_time_by_sample_number(0)
            end
        end
        ...
        describe "#add_sample" do
            it "appends the sample to the index" do
                stream_index.add_sample(0, 10)
                assert_equal 0, stream_index.internal_time_by_sample_number(0)
                assert_time_equal Time.at(0, 10), stream_index.time_by_sample_number(0)
            end
            it "sets the base time if it not already set" do
                stream_index.add_sample(0, 10)
                assert_equal 10, stream_index.base_time
            end
            it "does not reset the base time if it has already been set" do
                stream_index.add_sample(0, base_time = 10)
                stream_index.add_sample(0, base_time + 1)
                assert_equal 10, stream_index.base_time
                assert_equal 1, stream_index.internal_time_by_sample_number(1)
                assert_equal Time.at(0, 11), stream_index.time_by_sample_number(1)
            end
        end
        ...
        describe "#sample_number_by_time" do
            ...
            before do
                ...
                stream_index.add_sample(1, base_time + 10)
                stream_index.add_sample(2, base_time + 20)
            end

            it "returns the earliest sample whose time is not before the given time" do
                assert_equal 1, stream_index.sample_number_by_time(Time.at(0, base_time + 5))
            end
            it "returns 0 if the time is before the first sample" do
                assert_equal 0, stream_index.sample_number_by_time(Time.at(0, base_time - 1))
            end
            it "returns size if the time is after the last sample" do
                assert_equal 3, stream_index.sample_number_by_time(Time.at(0, base_time + 30))
            end
        end
        ...
        describe "#file_position_by_sample_number" do
            before do
                stream_index.add_sample(0, 10)
                stream_index.add_sample(1, 11)
                stream_index.add_sample(2, 12)
            end
        ...
        describe "#concat" do
            ...
            it "shifts the index so that find-by-time works for the new entries" do
                stream_index.concat(new_index)
                assert_equal 3, stream_index.sample_number_by_time(Time.at(0, base_time + 3))
            end
            it "uses the receiver's base time to encode the new entries" do
                new_index.base_time = base_time - 1
                stream_index.concat(new_index)
                assert_equal base_time, stream_index.base_time
                assert_equal 4, stream_index.internal_time_by_sample_number(3)
            end
